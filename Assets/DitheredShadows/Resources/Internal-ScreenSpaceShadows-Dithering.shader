...
Shader "Hidden/Bodhi Donselaar/Internal-ScreenSpaceShadows" {
Properties {
...
uniform float _DitherDirection;
...
{
	//Custom offset
	float3 offset = (-0.5 + (tex2D(_Noise, i.uv*(_ScreenParams.xy*_Noise_TexelSize.xy)).rgb));
    ...
    return camPos.xyz+ (offset * _DitherDirection);
}
